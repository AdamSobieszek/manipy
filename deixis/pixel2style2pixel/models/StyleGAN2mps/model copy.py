...
from .torch_utils.ops.upfirdn2d import upfirdn2d as upfirdn2d_ops
...
class EqualLinear(nn.Module):
    def __init__(self, in_dim, out_dim, bias=True, bias_init=0, lr_mul=1.0, activation=None, act_gain=None, act_clamp=None):
        super().__init__()
        ...
        self.act_gain = math.sqrt(2) if act_gain is None else act_gain
    ...
    def forward(self, x):
        # Compute affine transform without bias; add bias+activation via bias_act.
        out = F.linear(x, self.weight * self.scale, bias=None)

        if self.activation == 'fused_lrelu':
            b = (self.bias * self.lr_mul) if self.bias is not None else None
            return bias_act.bias_act(out, b, act='lrelu', gain=self.act_gain, clamp=self.act_clamp)
        else:
            # Linear pass-through (adds bias if present, no nonlinearity).
            b = (self.bias * self.lr_mul) if self.bias is not None else None
            return bias_act.bias_act(out, b, act='linear', gain=1.0, clamp=self.act_clamp)
...
# --- Switched to bias_act-based fused Leaky ReLU implementations ---

def fused_leaky_relu(x, b=None, act_gain=math.sqrt(2), act_clamp=None):
    """
    Fused bias + LeakyReLU using torch_utils.ops.bias_act.
    `b` is expected to be a 1D tensor of shape [C] for conv (NCHW) or [F] for linear (N x F).
    """
    return bias_act.bias_act(x, b, act='lrelu', gain=act_gain, clamp=act_clamp)


class FusedLeakyReLU(nn.Module):
    """
    Drop-in module replacement that uses bias_act for fused bias + LeakyReLU.
    """
    def __init__(self, channels, act_gain=math.sqrt(2), act_clamp=None):
        super().__init__()
    ...
    def forward(self, x):
        # bias_act accepts a 1D bias and handles broadcasting internally.
        return bias_act.bias_act(x, self.bias.to(x.dtype), act='lrelu', gain=self.act_gain, clamp=self.act_clamp)
