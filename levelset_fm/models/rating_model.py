...
class AlphaBetaRegressorNew(nn.Module):
    ...
    """
    def __init__(self, latent_dim: int = 512, w_avg: torch.Tensor = get_w_avg(), hidden_dim_multiplier: list = [8, 4, 2], dropout_rates: list = [0.1, 0.1, 0.1], exp_threshold: float = 10.0):
        super().__init__()
...
def load_trust_model_ensemble(dim_name, ensemble_size, checkpoint_dir, device, dtype=torch.float32):
    ...
    if ensemble_size == 0:
        ...
        for path in [f"model_new_{dim_name}{suffix}.pt", f"{dim_name}{suffix}.pt"]:
            ckpt_path = os.path.join(checkpoint_dir, path) # Match naming
            if os.path.exists(ckpt_path):
                model_instance = AlphaBetaRegressorNew().to(device, dtype)
    ...
    for i in range(1 if ensemble_size == 0 else ensemble_size):
        ...
        for path in [f"model_{dim_name}{suffix}.pt", f"{dim_name}{suffix}.pt"]:
            ckpt_path = os.path.join(checkpoint_dir, path) # Match naming
            if os.path.exists(ckpt_path):
                model_instance = AlphaBetaRegressor(dim=config.TRUST_MODEL_DIM_INTERNAL).to(device, dtype)
                return model_instance

        if not os.path.exists(ckpt_path):
             # Try alternative naming from the notebook
             ckpt_path_alt = os.path.join(checkpoint_dir, f"{dim_name}{suffix}.pt")
