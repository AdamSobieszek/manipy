...
from models.layers import Exponent # Import auxiliary layer

class AlphaBetaRegressor(nn.Module):
    """ Predicts Alpha and Beta parameters of a Beta distribution from W latent. """
    def __init__(self, dim=config.TRUST_MODEL_DIM_INTERNAL, output_activation=Exponent()): # Default to Exp activation
        super().__init__()
        self.w_avg = get_w_avg().detach() # Get w_avg, keep on its default device initially
        ...
        self.head = nn.Linear(config.LATENT_DIM_W, config.TRUST_MODEL_TARGET_DIM) # Output dim = 2
...
class EnsembleRegressor(nn.Module):
    ...
    def forward(self, x, output="a,b", *args, **kwargs):
        """ Averages the outputs of all models in the ensemble. """
        # Collect outputs from each model
        all_outputs = [model(x, output=output, *args, **kwargs) for model in self.models]
...
def load_trust_model_ensemble(dim_name, ensemble_size, checkpoint_dir, device):
    ...
    for i in range(ensemble_size):
        model_instance = AlphaBetaRegressor(dim=config.TRUST_MODEL_DIM_INTERNAL).to(device)
        # Construct path based on naming convention from training script
        # Example: model_trustworthy_v3.pt
        ckpt_path = os.path.join(checkpoint_dir, f"model_{dim_name}_v{3+i}.pt") # Match naming
        if not os.path.exists(ckpt_path):
             # Try alternative naming from the notebook
             ckpt_path_alt = os.path.join(checkpoint_dir, f"{dim_name}_v{3+i}.pt")
             if os.path.exists(ckpt_path_alt):
                 ckpt_path = ckpt_path_alt
             else:
                 print(f"Warning: Checkpoint not found for model {i} at {ckpt_path} or {ckpt_path_alt}. Using initialized weights.")
                 models.append(model_instance) # Add initialized model
                 continue # Skip loading for this one
    ...
    ensemble_model = EnsembleRegressor(models).to(device)
    ensemble_model.eval()
